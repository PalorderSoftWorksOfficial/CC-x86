...
function Decoder.new(cpu) return setmetatable({ cpu = cpu }, Decoder) end
...
function Decoder:fetch_i8() return U32.sign_extend8(self:fetch_u8()) end
function Decoder:fetch_i32() return U32.to_signed(self:fetch_u32()) end

---Decodes a ModR/M byte and any following displacement.
...
---Resolves the effective guest address represented by a ModR/M operand.
function Decoder:resolve_address(modrm)
    ...
    if modrm.rm == 4 then
        ...
        local address = 0
        if index ~= 4 then address = address + self.cpu.registers:get(index) * (2 ^ scale) end
